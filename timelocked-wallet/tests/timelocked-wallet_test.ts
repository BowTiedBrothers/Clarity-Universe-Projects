...
Clarinet.test({
    name: "Allows contract owner to lock amount",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const block = chain.mineBlock([
            ...
            [types.principal(beneficiary.address), types.uint(10), types.uint(amount)],
            deployer.address)
        ]);

        // The lock should be successful.
        block.receipts[0].result.expectOk().expectBool(true);
        // There should be an STX transfer of the amount specified.
